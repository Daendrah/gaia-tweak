export const BACKGROUND_FRAGMENT_SHADER = `
  ...
  float fbm(const vec3 p, const float scale, const int octaves) {
    float noise = 0.0;
    float freq = scale;
    float amplitude = 0.5;
  ...
  void main() {
    ...
    float mask0 = clamp(1.0 - height * u_gradientFalloffTop + density, 0.0, 1.0);
    float mask1 = clamp(1.0 - height * u_gradientFalloffBottom, 0.0, 1.0);
    vec3 layer0 = mix(u_backgroundColor, u_midColor, mask0);
    vec3 finalColor = mix(layer0, u_horizonColor, mask1 * mask1);
